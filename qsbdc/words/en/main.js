...
const host = 'http://word.qsbdc.com/'
const words = {}
const c = new Crawler({
    maxConnections: 2,
    rateLimit: 2000,
    ...
    for (let i = 2; i < rows.length - 1; i++) {
        ...
        if (enWordSpan && enWordSpan.children[0] && enWordSpan.children[0].data) {
            items.push({
                ...
                ph: phWordSpan && phWordSpan.children[0] && phWordSpan.children[0].data ? phWordSpan.children[0].data.replace(/(^\s*\[\s*|\s*\]\s*$)/g, '').split(/\s*\]\s*\[\s*/g) : [],
            ...
            console.log('Word Span Error:', enWordSpan.children[0].data, !phWordSpan, !cnWordSpan, !audioAnchor, !sampleAnchor)
        }
    }
...
async function dealWithItem(item, itemIndex, pageIndex, levelObj) {
    await insertItem(levelObj.level, item.en, item.ph.join('|'), item.cn, JSON.stringify(item.sentences))
    if (item.audio) await downloadAudio(item.en, item.audio)
    words[item.en] = item
    levelObj.total++
    count++
    console.log(count, levelObj.total, item.en, item.ph, item.cn, 'level:', levelObj.level, 'page:', pageIndex, 'index:', itemIndex)
}
