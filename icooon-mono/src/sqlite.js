...
exports.insertCat = function insertCat(id, name) {
    return new Promise(function (resolve, reject) {
        database.run(`INSERT OR IGNORE INTO cats (id, name, trans_en, trans_cn) VALUES (?, ?, '', '');`, id, name, function (err) {
            if (err) {
...
exports.insertWord = function insertWord(catid, item, imgIndexs) {
    const { en, jp } = item
    return new Promise(function (resolve, reject) {
        database.run(`
        INSERT OR REPLACE INTO words (cat, word, trans_jp, trans_cn, imgs)
        VALUES (?, ?, ?, '', ?);`, catid, en, jp, imgIndexs.join('|'), function (err) {
            if (err) {
